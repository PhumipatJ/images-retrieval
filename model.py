...
def extract_features_batch(model, dataloader, device, output_file):
    model.eval()
    all_global_feats = []
    all_local_feats = []
    all_img_names = []
    ...
    with torch.no_grad():
        for imgs, paths in tqdm(dataloader):
            ...
            # Global
            gfeat = feats['global'].cpu().numpy()
            all_global_feats.append(gfeat)

            # Local per image
            ldesc = feats['local']['descriptors'].cpu().numpy()
            att = feats['local']['attention'].cpu().numpy()
            for i in range(imgs.size(0)):
                all_local_feats.append({'descriptors': ldesc[i], 'attention': att[i]})

            # Paths
            paths = [str(p) for p in paths]
            all_img_names.extend(paths)

    all_global_feats = np.vstack(all_global_feats)
    np.savez(output_file,
             img_names=np.array(all_img_names),
             global_features=all_global_feats,
             local_features=np.array(all_local_feats, dtype=object))
    print(f"Features saved to {output_file}")

# Load fine-tuned model with local descriptors enabled
feature_model = DELG(pretrained=False, use_global=True, use_local=True).to(device)
feature_model.load_state_dict(torch.load("./saved_models/delg_finetuned.pth"))

os.makedirs("./features", exist_ok=True)
extract_features_batch(feature_model, train_loader, device, "./features/train.npz")
extract_features_batch(feature_model, val_loader, device, "./features/val.npz")
extract_features_batch(feature_model, test_loader, device, "./features/test.npz")